...
class MathChallengeGame
{
    ...
    static void AskQuestion()
    {
        ...
        try
        {
            ...
            {
                score += 10;
                Task.Run(() => PlaySound("Assets/correct.wav")); // Phat am thanh dung tren luong khac
                PrintMessage("Chinh xac! Ban duoc cong 10 diem.", true);
            }
            else
            {
                Task.Run(() => PlaySound("Assets/wrong.wav")); // Phat am thanh sai tren luong khac
    ...
    static void PlaySound(string filePath)
    {
        try
        {
            ...
            using (var outputDevice = new WaveOutEvent())
            {
                ...
                {
                    System.Threading.Thread.Sleep(10); // Doi am thanh phat xong
                }
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Khong the phat am thanh: {ex.Message}");
        }
    }
}
